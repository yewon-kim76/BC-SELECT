...
setwd("/gpfs/gsfs12/users/kimy35/SELECT/select_targeted/cox_sort_12_05/R_target_data")
source("./Ranking_Pairs4.R") ## import functions

###
nn = 50 ; # the number of partner genes
tcga_list <- readRDS(file = "./TCGA_genes.rds"); 
tcga_list <- tcga_list[1:19001] # remove PD1/PDL1
...
SL_AUC(data1 = GSE50948_3, data2 = GSE50948_4, drugs = "TRASTUZUMAB", drug_response = GSE50948$outcome, 
       pair = pairs)

# SDL
pairs <- intersect(unique(candidates_sdl$GENE_B), rownames(GSE50948_3))[1:nn]
SDL_AUC(data1 = GSE50948_3, data2 = GSE50948_4, drugs = "TRASTUZUMAB", drug_response = GSE50948$outcome, 
       pair = pairs)
...
SL_AUC(data1 = GSE37946_3, data2 = GSE37946_4, drugs = "TRASTUZUMAB", drug_response = GSE37946$outcome, 
       pair = pairs)

# SDL
pairs <- intersect(unique(candidates_sdl$GENE_B), rownames(GSE37946_3))[1:nn]
SDL_AUC(data1 = GSE37946_3, data2 = GSE37946_4, drugs = "TRASTUZUMAB", drug_response = GSE37946$outcome, 
       pair = pairs)
...
SL_AUC(data1 = GSE42822_3, data2 = GSE42822_4, drugs = "TRASTUZUMAB", drug_response = GSE42822$outcome, 
       pair = pairs)

# SDL
pairs <- intersect(unique(candidates_sdl$GENE_B), rownames(GSE42822_3))[1:nn]
SDL_AUC(data1 = GSE42822_3, data2 = GSE42822_4, drugs = "TRASTUZUMAB", drug_response = GSE42822$outcome, 
       pair = pairs)
...
SL_AUC(data1 = transneo_3, data2 = transneo_4, drugs = "TRASTUZUMAB", drug_response = transneo$outcome, 
       pair = pairs)
# SDL
pairs <- intersect(unique(candidates_sdl$GENE_B), rownames(transneo_3 ))[1:nn]
SDL_AUC(data1 = transneo_3, data2 = transneo_4, drugs = "TRASTUZUMAB", drug_response = transneo$outcome, 
       pair = pairs)
...
sub_gene <- intersect(tcga_list, rownames(GSE66399_L$mRNA_tcga))
GSE66399_L3 <- GSE66399_L$mRNA_tcga[rownames(GSE66399_L$mRNA_tcga) %in% sub_gene,] ; rm(sub_gene)
GSE66399_L4 <- GSE66399_L$mRNA_BC360[rownames(GSE66399_L$mRNA_BC360) %in% bc_gene_list,] ; dim(GSE66399_L3); dim(GSE66399_L4)
...
SL_AUC(data1 = GSE66399_L3, data2 = GSE66399_L4, drugs = "LAPATINIB",  drug_response = GSE66399_L$drug_response, pair = pairs)
...
SDL_AUC(data1 = GSE66399_L3, data2 = GSE66399_L4, drugs = "LAPATINIB",  drug_response = GSE66399_L$drug_response, pair = pairs)
#SDL_AUC(data1 = GSE66399_L3, data2 = GSE66399_L4, drugs = "TRASTUZUMAB",  drug_response = GSE66399_L$drug_response, pair = pairs)
# Focusing on ERBB2, BC-SELECT AUC : 0.663
...
SL_AUC(data1 = GSE194040_2, data2 = GSE194040_3, drugs = "VELIPARIB", 
...
SDL_AUC(data1 = GSE194040_2, data2 = GSE194040_3, drugs = "VELIPARIB", 
        drug_response = GSE194040_ISPY2$response$pCR, pair = pairs)
...
SL_AUC(data1 = GSE164458_2, data2 = GSE164458_3, drugs = "VELIPARIB", 
...
SDL_AUC(data1 = GSE164458_2, data2 = GSE164458_3, drugs = "VELIPARIB", 
        drug_response = dat.GSE164458$response$binary, pair = pairs)
...
SL_AUC(data1 = GSE173839_2, data2 = GSE173839_3, drugs = "OLAPARIB", 
...
SDL_AUC(data1 = GSE173839_2, data2 = GSE173839_3, drugs = "OLAPARIB", 
        drug_response = ISPY2_OLAPARIB$response$binary, pair = pairs)


rm(ISPY2_OLAPARIB, GSE173839_2, GSE173839_3, pairs)

# Figure: histogram of Monte Carlo experiments
control_t <- data.frame(control, rep("Control1", 5000)); colnames(control_t) <- c("AUC", "type")
bc1 <- data.frame(bc_metaAUC2, rep("Control2", 5000)); colnames(bc1) <- c("AUC", "type")
bc2 <- data.frame(bc_metaAUC4, rep("BC-SELECT", 5000)); colnames(bc2) <- c("AUC", "type")

DATA <- rbind(control_t, bc1, bc2)
auc <- data.frame(auc = c(AUC1, AUC2, AUC4), type = c("Control1","Control2", "BC-SELECT"))
ggplot(DATA, aes(x = AUC), fill = type) +
  geom_histogram(bins = 60) +  facet_grid(vars(type)) + 
  geom_vline(data=auc, aes(xintercept=auc, group = type, color="red"), linetype="dashed", size = 0.9) +
  theme(legend.position="None") + theme(strip.text = element_text(
    size = 11, color = "black")) + ylab("Count")
...
ggplot(data = df, aes(x=dataset, y = AUC, fill = method)) +
  ...
  geom_text(aes(label = AUC, y = AUC), position = position_dodge(width = 1),
            vjust = -0.5, size = 5) + ylab("AUC of ROC")
